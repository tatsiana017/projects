...
namespace CourseProject.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        [HttpPost]
        public async Task<ActionResult> BookingHall(BookingViewModel bookingViewModel)
        {
            var user = await UserManager.FindByIdAsync(User.Identity.GetUserId());
            if (ModelState.IsValid)
            {
                ...
                {
                    Id = Guid.NewGuid().ToString(),
                    HallId = bookingViewModel.RoomId,
                    DateFrom = bookingViewModel.DateFrom,
                    DateTo = bookingViewModel.DateTo
                ...
                {
                    Id = Guid.NewGuid().ToString(),
                    Check = db.BanquetingHall.Find(bookingViewModel.RoomId).Price,
                    EventId = events.Id,
                    UserId = user.Id,

                };
                db.Booking.Add(booking);
                db.BanquetingHall.Find(bookingViewModel.RoomId).Status = "Забронирован";
                db.SaveChanges();
                TempData["Message"] = "Бронирование оформлено успешно.";
                return RedirectToAction("SearchHotel", "Home");
            }
